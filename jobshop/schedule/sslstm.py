...
class Problem:
    ...
    def GenerateRandomProblem(self):
        ...
        if self.bool_generate_rondom_JSSP == True:
            ...
            self.p = np.array(
                [[29, 78, 9, 36, 98, 11, 62, 56, 44, 21], [43, 90, 75, 11, 69, 28, 46, 46, 72, 30],  ####49
    ...
    def PlotResult(self, num=0):
        random.seed(30)
        np.random.seed(30)
        result = {'id': '', 'color': '', 'x1': 0, 'x2': 0, 'y1': 0, 'y2': 0}
        sch_id = []
        color_list = []
        order_id = []
        x1_list = []
        ...
        for i in range(self.m):
            ...
            for j in range(self.n):
                ...
                x4 = mPoint1
                y4 = mText
                # 작동일자 들어오면 나중에 i 랑 j 지우기
                sch_id.append(datetime.datetime.today().strftime("%Y%m%d") + str(j) + str(i))
                color_list.append(colorbox[self.x[i, j, 0]])
        ...
        plt.ylabel('Machine')
        plt.tight_layout()
        # plt.savefig('/' + filedir + 'results/out.png', dpi=700)
        return result
...
# from keras.layers import (Conv2D, Dense, RepeatVector, Dropout, BatchNormalization, Flatten, Input, MaxPooling2D, LSTM,
#                           TimeDistributed)
...
def generating_features_labels(m, n, nubmers_of_problems):
    ...
    features_1D_bbm[np.isinf(features_1D_bbm)] = 0
...
def training_model(features, labels):
    ...
    #                                                                     random_state=42)
    X_train_ga, X_test_ga, y_train_ga, y_test_ga = train_test_split(features, labels, test_size=0.2,
                                                                    random_state=42)
    ss_lstm, his_sslstm = SS_lstm(X_train_ga[:, 0:18], y_train_ga, X_test_ga[:, 0:18], y_test_ga,
                                  X_train_ga[:, -1], X_test_ga[:, -1],
                                  filedir + "model/fusion/best.hdf5")
...
def solving_new_jssp(new_m, new_n):
    ...
    features_ft10_10by10_bbm[np.isinf(features_ft10_10by10_bbm)] = 0
    ###Step 1.1 Loading the model
    bestss_model_bbm = load_model(filedir + "model/fusion/best.hdf5")
    ###Step 1.2 Peparing the system-level features using K-means
    kmeans = KMeans(n_clusters=12).fit(
        normilization(features_ft10_10by10_bbm.reshape(features_ft10_10by10_bbm.shape[0], 18)))
    centroids = kmeans.cluster_centers_
    ...
    features_ft10_10by10_bbm_types = pd.DataFrame(
        normilization(features_ft10_10by10_bbm.reshape(features_ft10_10by10_bbm.shape[0], 18)))
    features_ft10_10by10_bbm_types["type"] = pred_types

    sslstm_bbm = bestss_model_bbm.predict(
        [normilization(np.asarray(features_ft10_10by10_bbm_types)[:, 0:18].reshape(new_m * new_n, 18, 1)),
    ...
    print('sstlsm : ', sslstm_mak_bbm)
    result = {}
    result = new_pro_ga.PlotResult()
    return result
...
def sslstm(result):
    ...
    new_n = 10
    result = {}
    result = solving_new_jssp(new_m, new_n)
    return result
